...
class HeaderLogo extends React.Component {
  ...
  render() {
    ...
    let keyNote = '';
    if (!fullscreen) {
      keyNote = <p className={classes.keynote}>Note: keyboard shortcuts are only available when the app is fullscreen.</p>;
    }

    const container1 = (
      <div className={classes.dialogDiv} style={{ maxHeight: Math.max(50, windowHeight - 310) }}>
        ...
        <div className={classes.dialogP}>
          <strong>Why: </strong>
          Viewing multiple slices of a dataset simultaneously is a simple but very powerful visual technique and provides a
          way to visualize data in greater detail, particularly when the dataset is large.
        </div>
        <div className={classes.dialogP}>
      ...
      <div className={classes.dialogDiv}>
        <div>
          ...
          <div style={{ width: '50%', display: 'block', float: 'left' }}>
            ...
            <p className={classes.dialogUl}>Swiping left and right will page the panels forward and backward</p>
          </div>
        </div>
      </div>
    );
    ...
    const container3 = (
      <div className={classes.dialogDiv}>
        ...
        <p>
          Thanks to Bill Cleveland for ideas upon which this is built, to Saptarshi Guha for creating a multi-panel plot
          viewer prototype many years ago that inspired initial work, and to Barret Schloerke for the introduction to React
    ...
    return (
      <div>
        <button type="button" onClick={this.handleOpen} className={classes.logo}>
          Trelliscope
...
export default connect(mapStateToProps)(injectSheet(staticStyles)(HeaderLogo));
