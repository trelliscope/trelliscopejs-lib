...
const DisplayList = ({
      ...
      <GridList cellHeight={180} cols={3} className={classes.gridList}>
        ...
        {displayGroups[k].map((i) => (
          ...
          >
            ...
            <GridListTileBar
              ...
              title={<div className={classes.gridTitle}>{di[i].name.replace(/_/g, ' ')}</div>}
              subtitle={
        ...
        {cfg.disclaimer ? (
          <GridListTile cols={cfg.disclaimer.cols}>
            <div className={classes.disclaimer} dangerouslySetInnerHTML={{ __html: cfg.disclaimer.text }} />
          </GridListTile>
        ) : (
          ''
        )}
  ...
  return <div className={classes.container}>{displayList}</div>;
};
...
const getRelDispPositions = (selectedDisplay, relDisps, displayInfo, contentHeight, contentWidth) => {
...
const mapDispatchToProps = (dispatch) => ({
  handleCheckbox: (i, selectedRelDisps, selectedDisplay, di, contentHeight, contentWidth, cfg, appId) => {
    ...
    const relDispPositions = getRelDispPositions(selectedDisplay, newRelDisps, di, contentHeight, contentWidth);
...
export default connect(mapStateToProps, mapDispatchToProps)(injectSheet(staticStyles)(DisplayList));
